...
for fc in DemandForecast.objects.all():
    if fc.predicted_demand >= thr_high:
        fc.demand_level         fc.demand_level         fc.demand_level         fc.demand_level         fc.demand_level         fc.demand_level         fc.demand_level         fc.demand_level         fc.demand_level         fc.demand_level         fc.demand_like    vail        fc.demand_level         fc.demand_level         fc.demand_ [        fc.demandavailability'])
for lvl in ['high', 'medium', 'low']:
    c = DemandForecast.objects.filter(demand_level=lvl).count()
    print(f'{lvl}: {c}')
ENDOFFEND
python fix_demand.py
